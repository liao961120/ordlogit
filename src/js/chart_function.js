function updateSimplex(dist_name, params) {
    if (dist_name != "orderedLogit") return
    const simplex = toSimplex(params.slice(1));
    var spans = document.querySelectorAll("span.simplex");
    for (let i=0; i<simplex.length; i++) {
        var num = Math.round(100*simplex[i]) / 100;
        if (num == 0) {
            spans[i].innerText = "0.00"
            continue
        }
        if (num == 1) {
            spans[i].innerText = "1.00"
            continue
        }    
        spans[i].innerText = `${num}`.padEnd(4, "0")
    }
    var Pcum = [ ...cumsum(simplex) ];
    Pcum.pop();
    const alpha =  Pcum.map(x => logit(x));
