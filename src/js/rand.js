    ...
    switch(dist_name) {
        ...
        case "orderedLogit": 
            var pdf = function(x, params) {
                // params = [ phi, bar1, bar2, bar3 ... ]
                const phi = params[0];
                const baseP = toSimplex( params.slice(1) );
                const p = [ 0, ...cumsum(baseP) ];
                const a = p.map(e => logit(e));
                // x as 1-indexed
                var pmf;
                pmf = logistic(a.slice(1)[x-1] - phi) - logistic(a[x-1] - phi);
                return pmf;
            };
            break;
